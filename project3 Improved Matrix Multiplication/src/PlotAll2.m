...
run('data/GEMM_BLOCKED_PACKED_AVX_2_OPENMP.m');  % 文件路径
...
run('data/GEMM_BLOCKED_PACKED.m');
...
figure;
hold on;
plot(sizes_IJK, gflops_IJK, 'b-o;GEMM\_BLOCKED\_PACKED\_AVX\_2\_OPENMP;', 'LineWidth', 4, 'MarkerSize', 3); % Increase line width and marker size
plot(sizes_IKJ, gflops_IKJ, 'g-s;OpenBLAS;', 'LineWidth', 4, 'MarkerSize', 3);
%plot(sizes_CBLAS, gflops_CBLAS, 'r-*;GEMM\_BLOCKED\_PACKED;', 'LineWidth', 4, 'MarkerSize', 3);
hold off;
...
filename = sprintf("gflops_GEMM_BLOCKED_PACKED_AVX_2_OPENMP_vs_OpenBLAS");
print(filename, '-dpng');
